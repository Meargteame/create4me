        ...
        <DashboardLayout>
            <div className="max-w-7xl mx-auto pb-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
                    <div>
                        <h1 className="text-3xl font-bold text-txt-primary tracking-tight">Discover Opportunities</h1>
                        <p className="text-txt-secondary mt-2 text-lg">Find campaigns that match your unique style and audience.</p>
                    </div>

                    {/* Search Bar */}
                    <div className="relative group w-full md:w-96">
                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <FaSearch className="text-txt-secondary group-focus-within:text-primary transition-colors" />
                        </div>
                        <input
                            type="text"
                            className="block w-full pl-11 pr-4 py-3 bg-input border border-border-color rounded-xl leading-5 placeholder:text-txt-secondary focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-transparent transition-all shadow-sm group-hover:shadow-md text-txt-primary"
                ...
                <div className="flex overflow-x-auto gap-3 pb-6 mb-2 no-scrollbar">
                    {['For You', 'Trending', 'High Budget', 'Quick Turnaround', 'Tech', 'Fashion', 'Beauty'].map(cat => (
                        <button
                            ...
                            className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-bold transition-all ${activeCategory === cat
                        ...
                        {loading ? (
                            ...
                            filteredCampaigns.map((campaign, index) => (
                                <motion.div
                                    ...
                                    key={campaign._id}
                                    className="group bg-card rounded-3xl p-1 border border-border-color shadow-sm hover:shadow-2xl hover:shadow-primary/5 transition-all duration-300 flex flex-col h-full"
                                >
                                    <div className="relative p-6 flex flex-col h-full rounded-[20px] bg-card z-10">
                                        {/* Brand & Save Header */}
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="flex items-center gap-3">
                                                <div className="w-10 h-10 rounded-full bg-onyx text-white flex items-center justify-center font-bold text-xs">
                                                    {campaign.brandName?.substring(0, 2).toUpperCase()}
                                                </div>
                                                <div>
                                                    <h4 className="font-bold text-txt-primary text-sm">{campaign.brandName}</h4>
                                                    <span className="text-[10px] text-txt-secondary uppercase tracking-widest font-bold">Posted 2d ago</span>
                                                </div>
                                            </div>
                                            <button
                                                onClick={(e) => toggleSave(campaign._id, e)}
                                                className="w-8 h-8 rounded-full bg-input hover:bg-primary/10 flex items-center justify-center text-txt-secondary hover:text-primary transition-colors"
                                            >
                                        ...
                                        <div className="flex gap-2 mb-3">
                                            {campaign.platforms?.map((p: string) => (
                                                <span key={p} className="w-8 h-8 rounded-full bg-input flex items-center justify-center border border-border-color">
                                                    {getPlatformIcon(p)}
                                                </span>
                                            ))}
                                        </div>
                                        ...
                                        <p className="text-txt-secondary text-sm mb-6 line-clamp-2 flex-grow">
                                            {campaign.description}
                                        </p>

                                        {/* Tags/Pills */}
                                        <div className="flex flex-wrap gap-2 mb-6">
                                            <span className="px-3 py-1 bg-green-500/10 text-green-600 text-xs font-bold rounded-lg flex items-center gap-1 border border-green-500/20">
                                                <FaDollarSign className="text-[10px]" />
                                                {campaign.budget?.toLocaleString()}
                                            </span>
                                            <span className="px-3 py-1 bg-input text-txt-secondary text-xs font-bold rounded-lg flex items-center gap-1 border border-border-color">
                                        ...
                                        <Link
                                            ...
                                            className="w-full py-3 rounded-xl bg-onyx text-white font-bold text-sm flex items-center justify-center gap-2 group-hover:bg-primary transition-all shadow-lg shadow-gray-200 group-hover:shadow-primary/25"
                                        >
