        ...
        <DashboardLayout>
            <div className="max-w-7xl mx-auto pb-12">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-10 gap-6">
                    <div>
                        <h1 className="text-3xl font-bold text-txt-primary tracking-tight">Find Creators</h1>
                        <p className="text-txt-secondary mt-2 text-lg">Connect with top-tier talent for your next campaign.</p>
                    </div>
                    ...
                    <div className="relative group w-full md:w-96">
                        ...
                        <input
                            type="text"
                            className="block w-full pl-11 pr-4 py-3 bg-input border border-border-color rounded-xl leading-5 placeholder-txt-secondary focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-transparent transition-all shadow-sm group-hover:shadow-md text-txt-primary"
                    ...
                    {['All Talent', 'Fashion', 'Tech', 'Lifestyle', 'Gaming', 'Food', 'Fitness'].map(cat => (
                        <button
                            ...
                            className={`whitespace-nowrap px-5 py-2.5 rounded-full text-sm font-bold transition-all ${activeCategory === cat
                                    ...
                                    : 'bg-card text-txt-secondary hover:bg-input border border-border-color'
                                }`}
                        >
                            {cat}
                        </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    <AnimatePresence>
                        {loading ? (
                            [1, 2, 3, 4].map(i => <div key={i} className="h-96 bg-input rounded-3xl animate-pulse"></div>)
                        ) : (
                            filteredCreators.map((creator, index) => (
                                <motion.div
                                    ...
                                    key={creator._id}
                                    className="bg-card rounded-3xl p-6 border border-border-color shadow-sm hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 transition-all duration-300 flex flex-col text-center items-center group relative overflow-hidden"
                                >
                                    {/* Top Pattern */}
                                    <div className="absolute top-0 left-0 w-full h-24 bg-gradient-to-b from-primary/5 to-card z-0"></div>
                                    ...
                                    <div className="relative z-10 w-24 h-24 mb-4">
                                        <img
                                            ...
                                            className="w-full h-full rounded-full object-cover border-4 border-card shadow-md group-hover:scale-105 transition-transform duration-300"
                                        />
                                        <div className="absolute bottom-1 right-1 bg-green-500 w-4 h-4 rounded-full border-2 border-card" title="Active Now"></div>
                                    </div>

                                    {/* Name & Badge */}
                                    <div className="relative z-10 mb-1">
                                        <h3 className="text-lg font-bold text-txt-primary flex items-center justify-center gap-1">
                                            {creator.userId?.name}
                                    ...
                                    {/* Bio */}
                                    <p className="relative z-10 text-txt-secondary text-sm mb-6 line-clamp-2 px-2 min-h-[40px]">
                                        {creator.bio}
                                    </p>

                                    {/* Stats */}
                                    <div className="relative z-10 grid grid-cols-2 w-full gap-2 mb-6">
                                        <div className="bg-input rounded-xl p-2">
                                            <div className="text-txt-primary font-bold">{creator.followers || '10k'}</div>
                                            <div className="text-[10px] text-txt-secondary uppercase font-bold">Followers</div>
                                        </div>
                                        <div className="bg-input rounded-xl p-2">
                                            <div className="text-txt-primary font-bold">{creator.engagement || '3.5%'}</div>
                                            <div className="text-[10px] text-txt-secondary uppercase font-bold">Engage</div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="mt-auto w-full relative z-10">
                                        <div className="flex justify-center gap-3 mb-5">
                                            {creator.platforms?.map((p: string) => (
                                                <div key={p} className="p-2 bg-input rounded-full hover:bg-card hover:shadow-sm transition-all shadow-sm">
                                                    {getPlatformIcon(p)}
                                                </div>
                                            ))}
                                        </div>
