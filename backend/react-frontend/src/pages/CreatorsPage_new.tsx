...
// Mock data for creators (will be replaced by API data)
...
export default function CreatorsPage_New() {
  ...
  const fetchCreators = async () => {
    try {
      ...
      showToastNotification("Failed to load creators");
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        >
          ...
          <p className="text-gray-600">
            Connect with talented Ethiopian creators • {creators.length}{" "}
            creators available
          </p>
        </motion.div>
                  ...
                  >
                    {paginatedCreators.map((creator, index) => (
                      <motion.div
                        ...
                        className={`bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-300 ${
                          viewMode === "list" ? "flex" : ""
                        }`}
                      >
                        {/* Creator Card Content */}
                        <div
                          className={`p-6 ${viewMode === "list" ? "flex-1 flex items-center gap-6" : ""}`}
                        >
                          {/* Avatar & Basic Info */}
                          <div
                            className={`flex ${viewMode === "list" ? "items-center gap-4" : "flex-col items-center"} mb-4`}
                          >
                            <div className="relative">
                              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center ring-4 ring-blue-100">
