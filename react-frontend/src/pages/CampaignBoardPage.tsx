...
import { FaSearch, FaClock, FaDollarSign, FaRocket, FaFilter, FaTimes } from '../components/icons';
...
export default function CampaignBoardPage() {
  ...
  const categories = ['Marketing', 'Content Creation', 'Social Media', 'Product Launch', 'Influencer', 'Photography', 'Video', 'Other'];
  ...
  const hasActiveFilters = searchTerm || budgetMin || budgetMax || selectedCategory || deadlineFilter || sortBy !== 'newest';
  ...
  let filteredCampaigns = campaigns.filter(campaign => {
    ...
    // Budget filter
    if (budgetMin && campaign.budget && campaign.budget < Number(budgetMin)) return false;
    if (budgetMax && campaign.budget && campaign.budget > Number(budgetMax)) return false;
    ...
    if (deadlineFilter && campaign.deadline) {
      const daysUntilDeadline = Math.ceil((new Date(campaign.deadline).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="mb-8 text-center">
          <h1 className="text-4xl font-bold text-gray-900">Find Your Next Collaboration</h1>
          <p className="text-lg text-gray-600 mt-2">Browse campaigns from top brands and apply to work with them.</p>
        </div>
        ...
        {!loading && filteredCampaigns.length > 0 && (
          <div className="mb-6 max-w-4xl mx-auto">
            <p className="text-sm text-gray-600">
              Showing <span className="font-semibold text-gray-900">{startIndex + 1}-{Math.min(endIndex, totalItems)}</span> of{' '}
              <span className="font-semibold text-gray-900">{totalItems}</span> {totalItems === 1 ? 'campaign' : 'campaigns'}
            </p>
          </div>
        )}
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              return (
                <div key={campaign.id} className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                  <div className="p-6">
                    {/* Header */}
                    ...
                    <div className="space-y-2 mb-4">
                      {campaign.budget && (
                        <div className="flex items-center text-sm text-gray-700">
                          <FaDollarSign className="mr-2 text-green-600" size={16} />
                          <span className="font-semibold">{campaign.budget.toLocaleString()} ETB</span>
                        </div>
                      )}
                      ...
                      {campaign.deadline && (
                        <div className={`flex items-center text-sm font-medium ${isUrgent ? 'text-red-600' : 'text-gray-700'}`}>
                          <FaClock className="mr-2" size={14} />
                          <span>
                            Deadline: {daysUntilDeadline !== null && daysUntilDeadline > 0
