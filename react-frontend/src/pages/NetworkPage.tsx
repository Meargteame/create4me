    ...
    try {
      ...
      if (response.success && response.connections) {
        ...
        setStats(response.stats || {
          ...
          connected: response.connections.filter((c: Connection) => c.status === 'connected').length,
          pending: response.connections.filter((c: Connection) => c.status === 'pending').length,
          suggested: response.connections.filter((c: Connection) => c.status === 'suggested').length,
          brands: response.connections.filter((c: Connection) => c.role === 'brand').length,
          creators: response.connections.filter((c: Connection) => c.role === 'creator').length
        })
      } else {
        // No data from backend
        setConnections([])
        setStats({ total: 0, connected: 0, pending: 0, suggested: 0, brands: 0, creators: 0 })
      }
      setLoading(false)
    } catch (error) {
      ...
      setStats({ total: 0, connected: 0, pending: 0, suggested: 0, brands: 0, creators: 0 })
      setLoading(false)
      showToastNotification('Failed to load connections. Please check your connection.')
    }
  }
